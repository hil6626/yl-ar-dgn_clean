...
```python
AR_NODES = [
    {"id": "ar-backend", "name": "AR Backend", "url": "http://localhost:5501"},
    {"id": "user-gui", "name": "User GUI", "url": "http://localhost:5502"}
]
```
...
class StatusReporter:
    def __init__(self, monitor_url="http://localhost:5500"):
...
AR_NODES_CONFIG = {
    "ar-backend": {
        ...
        "type": "backend",
        "health_url": "http://localhost:5501/health",
        "status_url": "http://localhost:5501/status",
        "check_interval": 30
    },
    "user-gui": {
        "name": "User GUI Application",
        "type": "frontend",
        "health_url": "http://localhost:5502/health",
        "check_interval": 30
    }
}
```
...
| 验证项 | 方法 | 预期结果 |
|--------|------|----------|
| AR-backend 健康检查 | `curl http://localhost:5501/health` | 返回 healthy |
| User GUI 状态查询 | `curl http://localhost:5502/status` | 返回 running |
