...
1️⃣ 全局关注：
- 全局上下文管理，确保模块间的联动性和数据一致性。
- 优先对现有脚本/代码进行优化，不随意生成新的脚本。
- 优化完成后清理沉积内容、缓存和无用文件。
- 每次优化部署的时候，确保优化内容放置在正确的目录。
- 每次优化完成以后会同步修正说明文档，保持说明文档与项目实际部署情况一致。
...
4️⃣ 跨域与接口设计：
- 检查前后端接口规范性、跨域配置是否正确。
- 输出防护/检测规则，例如 CORS、接口安全、返回数据类型。

5️⃣ 模板引擎与渲染：
- 检查模板引擎使用规范和渲染效率。
- 检查重复渲染、冗余 DOM 元素或逻辑冲突。
- 输出优化建议。

6️⃣ 表单验证：
- 检查表单前后端验证一致性。
- 输出防护/检测规则，包括输入类型、边界条件、必填项等。
...
8️⃣ 分页与性能：
- 检查分页逻辑是否合理、数据量大时的性能问题。
- 输出防护/检测规则，建议优化方案。

9️⃣ 事件冒泡与前端行为：
- 检查事件绑定方式、防止重复触发、阻止默认行为是否合理。
- 输出优化和防护建议。

🔹 输出要求：
- 每一部分必须包含：要点、检查点、可执行的防护/检测规则。
- 优化建议应直接可操作，优先改进现有脚本。
- 结构清晰，逻辑顺序完整，保证项目整体健壮性、兼容性和联动性。
- 每次完成优化修改以后，都会对优化以及修改的内容进行实际运行验证，确保功能落实.
